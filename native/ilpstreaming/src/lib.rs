// imports
use interledger::packet::ErrorCode;
...
fn encode_connection_close_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    let m = m.get("from_username").ok_or(error!("from_username is missing"))?;

    // transform
    let code = c.decode::<ErrorCode>().or(err!("could not decode to_username"))?; //Type ErrorCode
...
fn encode_connection_new_address<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    // get fields
    let sa = m.get("source_account").ok_or(error!("source_account is missing"))?;

    // transform
    let source_accounts = sa.decode::<&str>().or(err!("could not decode source_account"))?;

    //special decode
    let source_account = Address::from_str(source_accounts).or(err!("could not decode the source_account"))?;
...
fn encode_connection_asset_details_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    // get fields
    let sac = m.get("source_asset_code").ok_or(error!("source_asset_code is missing"))?;
    let sas = m.get("source_asset_scale").ok_or(error!("source_asset_scale is missing"))?;

    // transform
    let source_asset_code = sac.decode::<&str>().or(err!("could not decode source_asset_code"))?;
    let source_asset_scale = sas.decode::<u8>().or(err!("could not decode source_asset_code"))?;

    let pn = ConnectionAssetDetailsFrame { source_asset_code, source_asset_scale };
...
fn encode_connection_data_blocked_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    // transform
    let max_offset = mob.decode::<u64>().or(err!("could not decode max_offset"))?;
...
#[rustler::nif(schedule = "DirtyCpu")]
fn encode_connection_connection_max_stream_id_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    let m = arg.decode::<HashMap<String, Term>>().or(err!(
        "could not decode the stream_connection_connection_max_stream_id_frame to map<String,Term>"
    ))?;

    // get fields
    let msi = m.get("max_stream_id").ok_or(error!("max_stream_id is missing"))?;
  
    // transform
    let max_stream_id = msi.decode::<u64>().or(err!("could not decode max_stream_id"))?;
...
fn encode_connections_stream_id_blocked_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    // get fields
    let msis = m.get("max_stream_id").ok_or(error!("max_stream_id is missing"))?;
  
    // transform
    let max_stream_id = msis.decode::<u64>().or(err!("could not decode max_stream_id"))?;
...
fn encode_stream_close_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    let pn = StreamCloseFrame { stream_id, code, message};
...
fn encode_stream_max_money_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    // get fields
    let si = m.get("stream_id").ok_or(error!("stream_id is missing"))?;
    let rm = m.get("receive_max").ok_or(error!("receive_max is missing"))?;
    let tr = m.get("total_received").ok_or(error!("total_received is missing"))?;

  
    // transform
    let stream_id = si.decode::<u64>().or(err!("could not decode stream_id"))?;
    let receive_max = rm.decode::<u64>().or(err!("could not decode receive_max"))?; 
    let total_received = tr.decode::<u64>().or(err!("could not decode total_received"))?;

    
    let pn = StreamMaxMoneyFrame { stream_id, receive_max, total_received};
...
fn encode_stream_money_blocked_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    let ts = m.get("total_sent").ok_or(error!("total_received is total_sen"))?;
    ...
    let pn = StreamMoneyBlockedFrame { stream_id, send_max, total_sent};
...
fn encode_stream_data_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    let pn = StreamDataFrame { stream_id, offset, data};
...
fn encode_stream_max_data_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    let pn = StreamMaxDataFrame { stream_id, max_offset};
...
fn encode_stream_data_blocked_frame<'a>(env: Env<'a>, arg: Term) -> NifResult<Term<'a>> {
    ...
    let max_offset = mos.decode::<u64>().or(err!("could not decode max_offset"))?; 

    
    let pn = StreamDataBlockedFrame { stream_id, max_offset};
